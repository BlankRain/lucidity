...
(ns ^{:doc "A caching library for Clojure."
      :author "Fogus"}
  lucid.legacy.cache.tests
  (:use [lucid.legacy.cache] :reload-all)
  (:use [clojure.test])
  (:import (lucid.legacy.cache BasicCache FIFOCache LRUCache TTLCache LUCache
                               LIRSCache)
...
(deftest test-lru-cache
  ...
  (testing "regressions against LRU eviction before threshold met"
    (is (= {:b 3 :a 4}
           (-> (lucid.legacy.cache/lru-cache-factory {} :threshold 2)
               (assoc :a 1)
               (assoc :b 2)
               (assoc :b 3)
               (assoc :a 4)
    ...
    (is (= {:e 6, :d 5, :c 4}
           (-> (lucid.legacy.cache/lru-cache-factory {} :threshold 3)
               (assoc :a 1)
               (assoc :b 2)
               (assoc :b 3)
               (assoc :c 4)
...
(deftest test-lu-cache
  ...
  (testing "regressions against LRU eviction before threshold met"
    (is (= (-> (lucid.legacy.cache/lu-cache-factory {} :threshold 2)
               (assoc :a 1)
               (assoc :b 2)
               (assoc :b 3)
               (assoc :a 4))
    ...
    (is (= (-> (lucid.legacy.cache/lu-cache-factory {} :threshold 3)
               (assoc :a 1)
               (assoc :b 2)
               (assoc :b 3)
               (assoc :c 4)
    ...
    (is (= {:a 1 :b 3}
           (-> (lucid.legacy.cache/lu-cache-factory {} :threshold 2)
               (assoc :a 1)
               (assoc :b 2)
               (assoc :b 3)
               .cache)))))
