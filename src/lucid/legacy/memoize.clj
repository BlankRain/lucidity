...
(ns lucid.legacy.memoize
  ...
  **Manipulable memoization**
  ...
  (:require [lucid.legacy.cache :as cache]))
...
(deftype PluggableMemoization [f cache]
  cache/CacheProtocol
  (has? [_ item]
    (lucid.legacy.cache/has? cache item))
  (hit  [_ item]
    (PluggableMemoization. f (lucid.legacy.cache/hit cache item)))
  (miss [_ item result]
    (PluggableMemoization. f (lucid.legacy.cache/miss cache item result)))
  (evict [_ key]
    (PluggableMemoization. f (lucid.legacy.cache/evict cache key)))
  (lookup [_ item]
    (lucid.legacy.cache/lookup cache item))
  (seed [_ base]
    (PluggableMemoization. f (lucid.legacy.cache/seed cache base)))
  Object
...
(defn memo-clear!
  ...
  ([f]
     (when-let [cache (cache-id f)]
       (swap! cache (constantly (lucid.legacy.cache/seed @cache {})))))
  ([f args]
     (when-let [cache (cache-id f)]
       (swap! cache (constantly (lucid.legacy.cache/evict @cache args))))))
...
(defn memo-swap!
  ...
  (when-let [cache (cache-id f)]
    (swap! cache
           (constantly (lucid.legacy.cache/seed @cache
                             (into {}
                                   (for [[k v] base]
                                     [k (reify
                                          clojure.lang.IDeref
       ...
       (with-meta
        (fn [& args]
          (let [cs  (swap! cache through* f args)
                val (lucid.legacy.cache/lookup cs args)]
...
(defn memo
  "Used as a more flexible alternative to Clojure's core `memoization`
   ...
   You can access the memoization cache directly via the `:lucid.legacy.memoize/cache` key
...
(defn ^{:private true} !! [c]
  (println "WARNING - Deprecated construction method for"
           ...
           (str "(lucid.legacy.memoize/" c " function <base> <:" c "/threshold num>)")))
...
(defmacro ^{:private true} massert [condition msg]
  `(when-not ~condition
      (throw (new AssertionError (str "lucid.legacy.memoize/" ~msg "\n" (pr-str '~condition))))))
...
(def-deprecated fifo
  "DEPRECATED: Please use lucid.legacy.memoize/fifo instead."
  ([f] (memo-fifo f 32 {}))
...
(defn fifo
  "Works the same as the basic memoization function (i.e. `memo`
   ...
   Observe the following:
       ...
       (require '[lucid.legacy.memoize :as memo])
...
(def-deprecated lru
  "DEPRECATED: Please use lucid.legacy.memoize/lru instead."
  ([f] (memo-lru f 32))
...
(def-deprecated ttl
  "DEPRECATED: Please use lucid.legacy.memoize/ttl instead."
  ([f] (memo-ttl f 3000 {}))
...
(def-deprecated lu
  "DEPRECATED: Please use lucid.legacy.memoize/lu instead."
  ([f] (memo-lu f 32))
