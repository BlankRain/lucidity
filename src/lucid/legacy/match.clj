(ns lucid.legacy.match
  (:refer-clojure :exclude [compile])
  (:use [lucid.legacy.match.protocols])
  (:require [clojure.set :as set]
            [lucid.legacy.analyzer :as ana]
            [lucid.legacy.analyzer.jvm :as ana-jvm]
            [lucid.legacy.analyzer.passes.jvm.annotate-loops :as loops])
  (:import [java.io Writer]
           [lucid.legacy.match.protocols IExistentialPattern IPseudoPattern]))
...
(defn backtrack-expr []
  (if *clojurescript*
    ...
    `(throw lucid.legacy.match/backtrack)))

(defn backtrack-sym []
  (if *clojurescript*
    'cljs.core.match/backtrack
    'lucid.legacy.match/backtrack))
...
;; lucid.legacy.match.array and lucid.legacy.match.bits for
...
(defn dag-clause-to-clj [occurrence pattern action]
  (let [test (if (instance? lucid.legacy.match.protocols.IPatternCompile pattern)
