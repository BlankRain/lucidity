...
(defn through
  ...
  ([wrap-fn value-fn cache item]
    (if (lucid.legacy.cache/has? cache item)
      (lucid.legacy.cache/hit cache item)
      (lucid.legacy.cache/miss cache item (wrap-fn #(value-fn %) item)))))
...
(defn fifo-cache-factory
  ...
  (lucid.legacy.cache/seed (FIFOCache. {} clojure.lang.PersistentQueue/EMPTY threshold) base))
...
(defn ttl-cache-factory
  ...
  {:pre [(number? ttl) (<= 0 ttl)
         (map? base)]}
  (lucid.legacy.cache/seed (TTLCache. {} {} ttl) base))
